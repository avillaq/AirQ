...
if (!CESIUM_TOKEN) {
  console.error('❌ Missing NEXT_PUBLIC_CESIUM_TOKEN environment variable');
}
const AVATAR_STATES = {
...
const InteractiveMap = () => {
  ...
  const updateAQIData = async (lon, lat) => {
    ...
    let aqiValue = 50;
    if (data) {
      aqiValue = data.aqi + (Math.random() * 20 - 10);
      setLocationName(`${lat.toFixed(3)}, ${lon.toFixed(3)}`);
    } else {
      setLocationName("");
    }
    setAqi(Math.max(20, Math.min(200, aqiValue)));
  };
      ...
      <div className="status-panels">
        <div className="character-panel">
          ...
          <div className={`aqi-indicator ${getAQIClass(aqi)}`}>
            AQI: {getAQIText(aqi)} ({Math.round(aqi)})
          </div>
