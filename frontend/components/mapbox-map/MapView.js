...
export default function MapView({
  ...
  getAirQualityPoints()
    .then(geojson => {
      ...
      if (src) {
        src.setData(geojson);
        console.log('✅ Datos cargados:', geojson.features.length, 'puntos');
      }
    })
    .catch(err => {
      console.warn('Error cargando datos, usando mock:', err.message);
      ...
      const mockData = {
        ...
        features: [
          {
            type: 'Feature',
            properties: { id: 'mock-delhi', name: 'New Delhi, India', aqi: 185, pm25: 120 },
            geometry: { type: 'Point', coordinates: [77.2245, 28.6358] }
          },
          {
            type: 'Feature',
            properties: { id: 'mock-london', name: 'London, UK', aqi: 45, pm25: 15 },
            geometry: { type: 'Point', coordinates: [-0.1278, 51.5074] }
          },
          {
            type: 'Feature',
            properties: { id: 'mock-tokyo', name: 'Tokyo, Japan', aqi: 65, pm25: 22 },
  ...
  return (
    <div className="map-wrapper">
      ...
      {historicalData ? (
        ...
        airQualityData && <CompactMetrics data={airQualityData} locationName={locationName} />
      )}
...
function CompactMetrics({ data, locationName }) {
